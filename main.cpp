...
{
    log("%s(path=\"%s\", flags:%s, fi:{%s})", __func__, path, LOGBIT(OPEN_FLAGS, fi->flags), LOGFI(fi));
    INodePtr inode = g_cache.GetINode(path);
    if (!inode)
        return -ENOENT;
 ...
 * should be flushed, not the meta data
 */
int mzfs_fsyncdir(const char *path, int, struct fuse_file_info *fi)
{
    log("%s(%s)", __func__, path);
    return 0;
}
...
{
    log("%s(path=\"%s\", ioctl_flags:%s)", __func__, path, LOGBIT(IOCTL_FLAGS, flags));
    return 0;
}
          ...
          struct fuse_file_info *fi)
{
    ...
    log("fi:{%s} open_flags:%s", LOGFI(fi), LOGBIT(OPEN_FLAGS, fi->flags));
    INodePtr inode = g_cache.GetINode(path);
    if (!inode)
        return -ENOENT;
 ...
 * interesting for network filesystems and similar.
 */
int mzfs_flock(const char *path, struct fuse_file_info *fi, int op)
{
    log("%s(path=\"%s\", fi->flags:%s)", __func__, path, LOGBIT(B_FI, fi->flags));
    return 0;
}
